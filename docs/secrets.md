...
/beszel     DOMAIN, KEY, TOKEN
```

`/beszel`'s two are named `KEY` and `TOKEN` rather than `BESZEL_*` on purpose:
`bootstrap.sh` check 6d matches the names in `deploy.sh`'s table against the
running container's environment verbatim, and a renamed variable is not flagged
as blank — it is silently not checked. Paths are per stack, so the generic names
cannot collide.

`/beszel` is the one folder that cannot be filled in ahead of time. Its two
values are generated by the Beszel hub on first run, so the hub is started once
outside `deploy.sh` and the folder is populated from what it prints. See
[beszel.md](beszel.md#first-deploy).
