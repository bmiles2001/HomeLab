...
stacks/
  ...
  satisfactory/       game server. the only stack besides caddy that publishes
                      ports, because the game speaks raw udp. two more ordinary
...
# 5. Monitoring. Last, so it has something to watch. This one is TWO phases -
#    the agent's credentials are generated by the hub, so they cannot be in
#    Infisical before the hub's first run and deploy.sh would refuse. Start the
#    hub alone, create the account, then deploy properly:
#      cd stacks/beszel && docker compose up -d beszel
#      ... create admin, Add System, copy the key and token into Infisical ...
#      ./scripts/deploy.sh beszel
#    Full runbook: docs/beszel.md#first-deploy.
```
